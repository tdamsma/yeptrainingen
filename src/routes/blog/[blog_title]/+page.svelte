    ...
    .row
      .col-lg-9.col-md-8.col-sm-12
        .container
          .text-center.mb-4
            +if('data.img')
              enhanced:img.img-fluid(class="post-title-img" src="{imageModules[`/content/blog/${data.img}`].default}" alt="{data.alt}")
          h2 {data.title}
      ...
      .col-lg-3.col-md-4.col-sm-5.sidebar
        +each('data.surroundingDocuments as surroundingdata')
          .card.mt-5.overflow-hidden.bg-dark.text-white
            enhanced:img.card-img-top.rounded-0(src="{imageModules[`/content/blog/${surroundingdata.meta.img}`].default}"	 alt="{surroundingdata.meta.alt}")
            .card-body.p-3

  ...
  <div class="flexcontainer-wrapper">
    <div class="flexcontainer">
      {#each selectedOpdrachtgevers as opdrachtgever (opdrachtgever.opdrachtgever)}
        <div class="panel" transition:slide={{ delay: 0, duration: transitionTimeMS, easing: linear, axis: 'x' }}>
          {#if opdrachtgever.url !== ""}
            <a href={opdrachtgever.url}>
              <enhanced:img src={imageModules[`${opdrachtgever.name}.png`].default} alt={opdrachtgever.opdrachtgever} title={opdrachtgever.opdrachtgever} />
            </a>
          {:else}
            <enhanced:img src={imageModules[`${opdrachtgever.name}.png`].default} alt={opdrachtgever.opdrachtgever} title={opdrachtgever.opdrachtgever} />
          {/if}
...
const opdrachtgevers: Opdrachtgever[] = Object.entries(opdrachtgeverModules)
    .map(([path, module]) => ({
